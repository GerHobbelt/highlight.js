...
export default function(hljs) {
  ...
  function getStr() {
    return 'uint' + Array(256 / 8).fill().map(function(_, i) {
        (i + 1) * 8
      }).join(' uint') + ' ' +
      'uint' + Array(256 / 8).fill().map(function(_, i) {
        (i + 1) * 8
      }).join('[] uint') + ' ' +
      'int' + Array(256 / 8).fill().map(function(_, i) {
        (i + 1) * 8
      }).join(' int') + ' ' +
      'int' + Array(256 / 8).fill().map(function(_, i) {
        (i + 1) * 8
      }).join('[] int') + ' ' +
      'bytes' + Array(32).fill().map(function(_, i) {
        i + 1
      }).join(' bytes') + ' ' +
      'bytes' + Array(32).fill().map(function(_, i) {
        i + 1
      ...
      'ufixed' + [].concat.apply([], Array(32).fill().map(function(_, i) {
        Array(81).fill().map(function(_, j) {
          ((i + 1) * 8) + 'x' + j
        })
      })).join(' ufixed');
  ...
  var KEYWORDS = {
    keyword: 'anonymous as assembly break constant continue do delete else external for hex if ' +
    ...
    built_in: 'block msg tx now suicide selfdestruct addmod mulmod sha3 keccak256 log ' +
